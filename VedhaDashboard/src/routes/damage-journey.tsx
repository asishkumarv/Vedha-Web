...
export const Route = createFileRoute("/damage-journey")({
  component: DamageJourneyPage,
});

function DamageJourneyPage() {
  ...
  return <AdminLayout><DamageJourney /></AdminLayout>;
}

function DamageJourney() {
  ...
  return (
    <div className="space-y-4 w-full max-w-4xl">
      <div>
        <h2 className="text-xl sm:text-2xl font-bold text-foreground" style={{ fontFamily: "var(--font-heading)" }}>The Damage Journey</h2>
        <p className="text-muted-foreground text-sm mt-1">Edit the remaining {sections.length} content sections on your homepage.</p>
      </div>
            ...
            <AccordionItem key={section.id} value={section.id} className="border rounded-lg px-3 sm:px-4">
              <AccordionTrigger className="text-sm font-medium hover:no-underline">
                <span className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-destructive/10 text-destructive text-xs flex items-center justify-center font-bold">{idx + 1}</span>
